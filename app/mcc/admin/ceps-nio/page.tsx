  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {job?.problems?.length ? <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-semibold text-amber-900">Download bloqueado até revisão</p><ul className="mt-2 list-disc pl-5 text-sm text-amber-800">{job.problems.map((p) => <li key={p}>{p}</li>)}</ul></div> : null}
        {completed && <div className="mt-5 flex flex-wrap gap-3"><Button onClick={() => download("csv")} disabled={!job.valid}><Download className="size-4" />Baixar CSV validado</Button><Button variant="outline" onClick={() => download("audit")}><Download className="size-4" />Baixar auditoria</Button></div>}
      </div>
