...
import { Wifi, Loader2, Mail, Lock } from "lucide-react";

export default function LoginPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const callbackUrl = searchParams.get("callbackUrl") ?? "/mcc";
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 px-4">
      ...
      <div className="relative w-full max-w-[400px]">
        ...
        <p className="mt-6 text-center text-sm text-blue-200/40">
          Não tem conta?{" "}
          <a href="/mcc/register" className="text-blue-400 hover:text-blue-300 hover:underline">
            Cadastre-se
          </a>
        </p>
      </div>
